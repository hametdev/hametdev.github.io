...
for f in files:
    ...
    # features container centering
    c = re.sub(r'class="flex flex-col gap-4 pb-8 max-w-7xl w-full"', 'class="flex flex-col gap-4 pb-8 max-w-[1000px] mx-auto w-full items-center text-center"', c)
    c = re.sub(r'class="text-taupe text-\[17px\] md:text-lg max-w-4xl leading-relaxed"', 'class="text-taupe text-[17px] md:text-lg max-w-4xl leading-relaxed"', c) # keep it
    
    # faq container
    c = re.sub(r'<section class="relative py-24 px-4 md:px-10 flex justify-center"[^>]*id="faq">', '<section class="relative py-28 px-4 md:px-20 bg-neutral-50 flex flex-col items-center" id="faq">', c)
    c = re.sub(r'<section class="relative py-24 px-4 md:px-20 flex justify-center"[^>]*id="faq">', '<section class="relative py-28 px-4 md:px-20 bg-neutral-50 flex flex-col items-center" id="faq">', c)
    
    # pricing container (if faq was joined)
    c = re.sub(r'<section class="relative py-24 px-4 md:px-10 flex justify-center"[^>]*id="pricing">', '<section class="relative py-28 px-4 md:px-20 bg-[#EDEDED]" id="pricing">', c)
    
    # faq title center
    c = re.sub(r'<div class="flex flex-col max-w-[^>]*w-full gap-12">', '<div class="flex flex-col max-w-[1000px] w-full gap-12">', c)
    c = re.sub(r'<h2 class="text-charcoal text-4xl mb-4">', '<h2 class="text-charcoal text-4xl md:text-5xl font-medium font-display mb-4 text-center self-center">', c)
    
    # append empty grid icons properly before the closing elements of features section
    if "Empty Grid Icons" not in c:
        c = c.replace('                            </div>\n                        </div>\n                        </div>\n\n                    </div>\n                </div>\n            </section>', '                            </div>\n                        </div>\n                        </div>\n' + empty_icons_html + '\n                    </div>\n                </div>\n            </section>')
    
    with open(f, 'w', encoding='utf-8') as fh:
        fh.write(c)
    print("Done classes for", f)
